...
<xsl:template name="next-group-number">
  ...
  <xsl:variable name="properties">
    <xsl:call-template name="next-property-number"/>
  </xsl:variable>
  <xsl:value-of select="$collections+$groups+$properties"/>
</xsl:template>
...
<xsl:template name="next-property-number">
  <xsl:variable name="properties">
  <xsl:for-each select="ancestor-or-self::property|preceding::property">
    <xsl:if test="position()=last()">
      <xsl:value-of select="position()"/>
    </xsl:if>
  </xsl:for-each>
  ...
  <xsl:choose>
    <xsl:when test="string(number($properties))='NaN'">
  <xsl:value-of select="0"/>
    </xsl:when>
    <xsl:otherwise>
...
<!-- If typing nodes then label the node and add the class URL -->
      <xsl:if test="$type-nodes!=0">
        ...
        <xsl:text>" [label="</xsl:text>
        <xsl:value-of select="$property"/>
        <xsl:text>", color=lightgoldenrod, URL="</xsl:text>
        <xsl:value-of select="$uri-rdf-schema"/>
        <xsl:value-of select="substring-after($property, 'rdf:')"/>
        <xsl:text>"];</xsl:text>
        <xsl:value-of select="$nl"/>
      </xsl:if>

<!-- Draw the edges for the list item -->
      <xsl:variable name="group-name">
        <xsl:value-of select="'Item'"/>
      </xsl:variable>
      <xsl:variable name="group">
...
<!-- Now process the child elements -->
      <xsl:for-each select="*">
        ...
        <xsl:if test="$type-nodes=0 or not(name(.)='type')">
<!--
...
<!-- Process <property> child elements -->
          <xsl:when test="name(.)='property'">
            ...
            <xsl:text>" [label="</xsl:text>
            <xsl:value-of select="$yads-ns-has"/>
            <xsl:text>:property", URL="</xsl:text>
            <xsl:value-of select="$uri-yads-schema"/>
...
<!-- Process literal property child elements -->
<!--
          <xsl:when test=".=access|detail|directive|label|role|service|type">
-->
          <xsl:otherwise>
            <xsl:if test="$type-nodes=0 or not(name(.)='type')">
...
<!-- Start and end subgraph templates -->
  <xsl:template name="start-subgraph">
    ...
    <xsl:text>";</xsl:text>
    <xsl:value-of select="$nl"/>
    <xsl:text>  fontname=arial; fontsize=11; fontcolor="black";</xsl:text>
    <xsl:value-of select="$nl"/>
...
<!--
  Match literal property elements - promote "type" attribute to appropriate
