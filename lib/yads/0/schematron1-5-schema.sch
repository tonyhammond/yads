...
<schema xmlns="http://www.ascc.net/xml/schematron"
	...
	xml:lang="en" icon="http://www.ascc.net/xml/resource/schematron/bilby.jpg"
	fpi="+//IDN ascc.net//SGML XML Schematron 1.5 Schema for Schematron 1.5//EN"
	schemaVersion="2001/06/12"  version="1.5"
        defaultPhase="#ALL"
>
	<!--xmlns:xsi="http://www.w3.org/2000/10/XMLSchema-instance" xsi:schemaLocation="http://www.ascc.net/xml/schematron
	...
	These rules may be handy for beginners with starting documents.</p>
		<rule context="/">
			<report test="//schema/pattern/rule"  diagnostics="willBreak namespace"   icon="ry1.gif"
			>Your schema does not use namespaces.</report>
			<assert test="//sch:schema"  diagnostics="namespace"   icon="ry1.gif"
			>A Schematron schema should have a schema element. </assert>
                        <report test="count(//sch:schema) > 1"  icon="Alien06.gif"
			...
			<assert test="//sch:schema/sch:pattern/sch:rule/sch:assert[@test] 
 			or //sch:schema/sch:pattern/sch:rule/sch:assert[@test]" 
	...
	<pattern name="Schematron Elements and Required Attributes" id="required">
		...
		icon="Alien06.gif"	>A <name/> element should contain a natural language sentence.</assert>
		...
		<rule context="sch:schema">
			<assert test="count(sch:*) = count(sch:title|sch:ns|sch:phase|sch:p|sch:pattern|sch:diagnostics|sch:phase)"			
			>The element <name/> should contain only the elements title, ns, phase, p, pattern, diagnostics or phase from the Schematron namespace.</assert>
		...
		<rule context="sch:emph">
			<assert test="parent::sch:p | parent::sch:diagnostic|
                                                            parent::sch:assert|parent::sch:report"
			 diagnostics="bad-parent"
			>The element <name/> should only appear inside a Schematron elements assert, report, p (paragraph) or diagnostic. It is equivalent to the HTML element of the same name.</assert>
		</rule>
		<rule context="sch:dir">
			<assert test="parent::sch:p | parent::sch:diagnostic |
                                                             parent::sch:assert|parent::sch:report"
			 diagnostics="bad-parent"
			>The element <name/> should only appear inside a Schematron elements assert, report, p (paragraph) or diagnostic.</assert>
			<assert test="@value and (@value='rtl' or @value='ltr')"
			>The attribute value of the <name/> element must be lowercase "rtl" or "ltr". It is equivalent to the HTML attribute of the same name.</assert>
		</rule>
	...
	<pattern name="Schematron Attributes" id="attributes" >
		...
		<rule context="sch:*">
			...
			<report test="@fpi and not(self::sch:schema or self::sch:phase or self::sch:pattern)"
			>The attribute fpi can only appear on the elements schema, fpi and phase. It is an ISO Formal Public Identifier.</report>
			...
			>The attribute name can only appear on the schema element. It is the namespace for the names in the role attributes.</report>
        ...
        <diagnostics>
		<diagnostic id="willBreak"
		>This system requires namespaces.
		</diagnostic>
		<diagnostic id="namespace"
		>Check that the correct namespace is being used: http://www.ascc.net/xml/schematron
		</diagnostic>
