...
public class DObject {
  ...
  private HashMap dataObject;	// Member type/value pairs keyed on index
  private ArrayList dataGroups;	// 
   ...
   *
   * @param doi  The public identifier for this DObject - the DOI.
   */
   ...
   * @param doi  The public identifier for this DObject - the DOI.
   * @param profile  The DOI Application Profile.
   * @param hierarchy  A resource hierarchy.
   */
...
*/
  ...
  /**
   * Returns the DObject member types within the current DObject instance.
   *
   * @return  List of DObject member types.
   */
  ...
  /**
   * Returns the DObject member values within the current DObject instance.
   *
   * @return  List of DObject member values.
   */
...
*/
  ...
  /**
   * Returns DObject Groups from the current DObject instance.
   *
   * @return  List of DObject Groups.
   */
  public void getGroups() {

    this.getGroups(null, null);
   ...
   * Returns DObject Groups from the current DObject instance.
   *
   * @param typeMatch  Member type to match.
   *
   * @return  List of DObject Groups.
   */
  public void getGroups(String typeMatch) {

    this.getGroups(typeMatch, null);
   ...
   *
   * @param typeMatch  Member type to match.
   * @param dataMatch  Member data to match.
   *
   * @return  List of DObject Groups.
   */
  public ArrayList getGroups(String typeMatch, String dataMatch) {
    ...
    while (st.hasMoreTokens()) {
      ...
      if (s.equals(DObjectMap.GROUP_OPEN)) { nGroups++; }
    }
    List[] groups = new ArrayList[nGroups];
    ...
    while (st.hasMoreTokens()) {
      ...
      if (s.equals(DObjectMap.GROUP_ALTERNATE_OPEN)
        ...
        groups[i].add(new String[] { type, data });

      }
    }

    for (i = 0; i < groups.length; i++) {
      group = (ArrayList)groups[i];
      addGroup = false;
      for (Iterator i_ = group.iterator(); i_.hasNext(); ) {
        member = (String[])i_.next();
        type = member[0];
        data = member[1];

        if (typeMatch.length() > 0) {
          if (type.equals(typeMatch)) {
          // if (re.match("/" + typeMatch + "/", type)) {
            if (dataMatch.length() > 0) {
              if (data.equals(dataMatch)) {
              // if (re.match("/" + dataMatch + "/", type)) {
                addGroup = true;
              }
            }
            else {
  ...
  /**
   * Dumps DObject Groups from the current DObject instance.
   */
  public void dumpGroups() {

    this.dumpGroups(null, null);
   ...
   * Dumps DObject Groups from the current DObject instance.
   *
   * @param type  Member type to match.
   */
  public void dumpGroups(String type) {

    this.dumpGroups(type, null);
   ...
   *
   * @param type  Member type to match.
   * @param data  Member data to match.
   */
  public void dumpGroups(String type, String data) {
    ...
    groups = this.getGroups(type, data);
    for (Iterator i_ = groups.iterator(); i_.hasNext(); ) {
      System.out.println(DObjectMap.GROUP_OPEN);
      members = (ArrayList)i_.next();
      for (Iterator j_ = members.iterator(); j_.hasNext(); ) {
        member = (String[])j_.next();
        System.out.println("  " + member[0] + " => " + member[1]);
      }
      System.out.println(DObjectMap.GROUP_CLOSE);
    }
  }
...
*/
   ...
   *
   * @param XML  An RDF/XML description.
   * @return  The DObject instantiated from this description.
   */
  public DObject fromXML(String XML) {
...
*/
    ...
    out.println("\n// 1. Method example: DObject#getMemberTypes()");
    out.println("");
    out.println("   List l = d.getMemberTypes();");
    ...
    out.println("\n// 2. Method example: DObject#getMemberValues()");
    out.println("");
    out.println("   List l = d.getMemberValues();");
    out.println("   for (Iterator i_ = l.iterator(); i_.hasNext(); ) {");
    out.println("     out.println(i_.next().toString());");
    out.println("   }");
    out.println("");
